...
FIRST_SPAN_IN_CHUNK_CHECK = SpanTagChecksLoader.load_span_tag_check(
    "./specifications/ddtrace/root-span-in-chunk-spec.json"
)
...
class TraceTagValidationCheck(Check):
    def check(self, trace):
        for i, span in enumerate(trace):
            ...
            with CheckTrace.add_frame(span_failure_message(span, i)):
                SpanTagValidationCheck(
                    span=span,
...
# class TraceTagValidatorCheck(Check):
