...
general_metadata_rules = SpanMetadataRules(
    name="GENERAL",
    required_tags=["duration", "resource", "span_id", "runtime-id", "process_id", "trace_id", "start"], 
    optional_tags=["version", "language", "error.msg", "error.type", "error.stack", "parent_id", "service", "env"]
)
internal_metadata_rules = SpanMetadataRules(
    name="INTERNAL",
    optional_tags=["_dd.p.dm", "_dd.agent_psr", "_dd.measured", "_dd.top_level", "_sampling_priority_v1", "_dd.tracer_kr"]
)
...
aiohttp_metadata_rules = SpanMetadataRules(
